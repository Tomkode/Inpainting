...
def sample_plot_image_for_inpainting(mask, original_img, img):
    ...
    return img            
@torch.no_grad()
def sample_plot_image_for_inpainting_with_resampling(mask, original_img, img, res_steps = 10, res_freq = 1):
    # Sample noise
    img_size = IMG_SIZE
    if img is None:
        img = torch.randn((1, 3, img_size, img_size), device=device)
...
# m = (img > 1) | (img < -1)
# print(m.sum().item())
# img = sample_plot_image_for_inpainting_with_resampling_faces(mask.to(device), image.to(device), img.to(device), 5, 25)
# #img = sample_plot_image_for_inpainting(mask.to(device), image.to(device), img.to(device))
# show_tensor_image(img[0].cpu())
# plt.show()
...
#     # Use center mask instead of random mask
#     image, mask = apply_center_mask(image, mask_size=0.3)  # 30% of image size
#     img = image.clone()
#     img = sample_plot_image_for_inpainting_with_resampling_faces(mask.to(device), image.to(device), img.to(device), 5, 15)
    
#     # Calculate metrics
#     metrics = calculate_metrics(original_image, img)
